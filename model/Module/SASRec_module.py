...
class SASRec_Model(nn.Module):
    def __init__(self, data, emb_size, max_len, n_blocks, n_heads, drop_rate, feature, datasetFile,strategy):
        ...
        self.random2= torch.rand_like(self.word_embeddings2).cuda()
        ...
        self.mapping2 = MLPS_for_reprogram2()
        tokenizer = [self.tokenizer5 ,self.tokenizer2 ,self.tokenizer3,self.tokenizer4,self.tokenizer_gpt2,self.tokenizer6]
        llm = [ self.llama, self.llm_model2, self.llm_model3, self.llm_model4,self.llm_modelgpt,self.llm_model6]
    ...
    def forward(self, seq, pos):
        ...
        for i in range(len(self.attention_layers)):
            if(i==0):
                seq_emb_moe = self.MoE(seq_emb, seq.cuda(),self.strategy,self.model_embedding_list)
...
class MoE(nn.Module):
    def __init__(self, input_dim, output_dim, llm, tokenizer, num_experts=6,dropout_rate=0.1, noisy_gating=False,
                 noise_epsilon=1e-2,
                 use_shared_expert: bool = False,
                 shared_group=(1, 2, 3),
                 shared_lambd: float = 1, ):
        ...
        txt_file_path = '/root/autodl-tmp/dataset/Amazon-Music/introduction_gpt4o.txt'
        ...
        for i in range(6):
            ...
            embeddings = llm[i].get_input_embeddings().cuda()(input_ids).detach().clone().requires_grad_(True)
...
\
    ...
    def forward(self, seq_emb, seq,strategy,word_embeddings_list):
        ...
        for i in range(self.num_experts):
            ...
            if strategy ==2:
                source_embeddings = self.process_and_select_grad(seq_emb, word_embeddings_list[i], self.dimension_mappings[i],
                                                             seq)
            if strategy ==1:
        ...
        if self.use_shared_expert and (self.shared_expert is not None):
            ...
            if shared_source is not None:
                shared_out = self.shared_expert(seq_emb, shared_source, shared_source,seq)
...
\
        ...
        mask = seq != 0
        A_sum = (A * mask.unsqueeze(-1)).sum(dim=1)
        item_count = mask.sum(dim=1).clamp(min=1)
        A_avg = A_sum / item_count.unsqueeze(-1)
        linear_mapping = linear_mapping.to(A_avg.device)
        B_mapped = linear_mapping(B)
        A = A_avg

        A_normalized = F.normalize(A, p=2, dim=1).to(torch.float16)
        ...
        top_k_indices = torch.topk(similarities,25, dim=1, largest=True).indices

        nearest_vectors = torch.stack([B[indices] for indices in top_k_indices])
...
\
    ...
    def encode_and_expand(self, word_embedding,i):
        ...
        nearest_indices = torch.topk(similarities, 10, dim=1, largest=True).indices.squeeze().to(word_embedding.device)
