...
import sys
from visualization.embeddingToWord import EmbeddingToWord
...
class BERT_Encoder(nn.Module):
    def __init__(self, data, emb_size, max_len, n_blocks, n_heads, drop_rate, feature, datasetFile,strategy):
        ...
        self.word_embeddings5_clone=self.word_embeddings5.clone()
        self.word_embedding_gpt2 = self.llm_modelgpt.get_input_embeddings().weight
        self.word_embedding_gpt2_clone=self.word_embedding_gpt2.clone()
        self.word_embedding_qwen = self.llm_model6.get_input_embeddings().weight
        self.word_embedding_qwen.clone=self.word_embedding_gpt2.clone()
...
class MoE(nn.Module):
    ...
    def forward(self, seq_emb,seq,strategy,word_embeddings_list):
        ...
        for i in range(self.num_experts):
            ...
            if strategy==2:
                source_embeddings = self.process_and_select_grad(seq_emb,word_embeddings_list[i],self.dimension_mappings[i], seq,i)
            if strategy==3:
...
\
    ...
    def process_and_select_grad(self, A: torch.Tensor, B: torch.Tensor, linear_mapping, seq, i):
\
\
\
\
...
\
        ...
        if (i == 1) and self.training and torch.is_grad_enabled():
            sim.retain_grad()
    ...
    def encode_and_expand(self, word_embedding, i):
        ...
        nearest_indices = torch.topk(similarities, 10, dim=1, largest=True).indices.squeeze().to(
