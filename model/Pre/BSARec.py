...
from model.Module.BSARec_module import BSARecModel
...
class BSARec(SequentialRecommender):
    def __init__(self, conf, training_set, test_set):
        super(BSARec, self).__init__(conf, training_set, test_set)

        args = OptionConf(self.config['BSARec'])
    ...
    def train(self):
        ...
        if self.round2 == 1:
            load_round1_and_enable_shared(
                model,
                '/root/autodl-tmp/fisher_cache/fisher_experts_epoch_BSA_beer20.pt',
                enable_shared=True
            )
